...
def train_funsd(model, criterion, optimizer, train_loader, config):
    ...
    for train_graph in train_loader:
        ...
        m = train_graph.edata['m'].to(device)

        x_pred, x_true, n_scores = model(train_graph, m, mask_rate = config['mask_rate'])
        ...
        wandb.log({'classification loss': n_loss.item()})
...
def validation_funsd(model, criterion, val_loader):
    ...
    with torch.no_grad():
        for val_graph in val_loader:
            ...
            m = val_graph.edata['m'].to(device)

            x_pred_val, x_true_val, n_scores_val = model(val_graph, m, mask_rate = 0.0)
...
def test_funsd(model, test_loader, criterion):
    ...
    with torch.no_grad():
        ...
        for test_graph in test_loader:
            ...
            m = test_graph.edata['m'].to(device)

            x_pred_test, x_true_test, n_scores_test = model(test_graph, m, mask_rate = 0.0)
...
def test_evaluation(model, train_loader, criterion, config):
    ...
    if config['kmeans_partition']:
        test_loader = edgesAggregation_kmeans_graphs(train = False, config = config)
    else:
...
def Sub_Graphs_masking_edges(config):
    # Loading data
    if config['kmeans_partition']:
    ...
    try:
        if config['network']['checkpoint'] is None:
            ...
            for epoch in range(config['epochs']):
                ...
                train_loss, macro, auc, accuracy_train = train_funsd(model, criterion, optimizer, train_loader, config)
                val_tot_loss, val_macro, val_auc, precision, accuracy_val = validation_funsd(model, criterion, val_loader)
                scheduler.step()
